...
public class ManageController implements Serializable {
    ...
    private void init() {
        ...
        try {
            for (Role role : RoleLocalServiceUtil.getUserRoles(td.getUserId())) {
                if (role.getName().equals("Redakteur") && RoleLocalServiceUtil.hasUserRole(role.getRoleId(), td.getUserId())) {
                    redakteur = true;
                }
            }
        ...
        /*
         * Sets caption according to create/edit metadata for dataset/app/document
         */
        if (metadataController.getMetadata().getTitle() == null || metadataController.getMetadata().getTitle().isEmpty()) {
            /*
             * Create
             */
            ...
            unknownLicenceText = metadataController.getMetadata().getLicence().getOther();
        ...
        /*
         * Fill licences accordingly to the metadata type: dataset, app, document
         */
        if (metadataController.getMetadata().getType().equals(MetadataEnumType.DATASET)
                || metadataController.getMetadata().getType().equals(MetadataEnumType.UNKNOWN)) {
        ...
        for (Licence licence : licences) {
            if (!licence.isOpen() && !licence.getTitle().equals(LanguageUtil.get(request.getLocale(), "od.licence.select"))) {
    ...
    public String submit() {
        ...
        for (Licence l : licences) {
            if (selectedLicence.equals(l.getName())) {
                ...
                if (unknownLicence) {
                    Licence oL = metadataController.getMetadata().getLicence();
                    oL.setOther(unknownLicenceText);
                    metadataController.getMetadata().setLicence(oL);
                }
                break;
            }
        }
        ...
        for (Category c : categories) {
            for (String cName : selectedCategories) {
        ...
        try {
            ...
            case APPLICATION:
                ...
                try {
                    metadataController.getMetadata().getImages().add(ImageGalleryUtils.uploadImage(appImage.getUploadedFile()).toString());
        ...
        try {
            try {
                ...
                // TODO: better errorhandling...
                LOG.error("Error on creating metadata: {}", metadataController.getMetadata().getName(), e.getMessage());
            }
            ...
            if (!editMode && metadataController.getMetadata().getType().equals(MetadataEnumType.APPLICATION)) {
                ...
                Metadata newApp;
                try {
                    newApp = appOdrClient.getMetadata(currentUser.getCurrentUser(), metadataController.getMetadata().getName());
                    ...
                    // TODO: better errorhandling...
                    LOG.error("Error on updating state of metadata to deleted: {}", metadataController.getMetadata().getName(),
                            e.getMessage());
                } catch (PortalException e) {
    ...
    private boolean unknownLicence(String name) {
        ...
        if (name.equals("other-open") || name.equals("other-closed") || name.equals("app_commercial") || name.equals("app_freeware")
    ...
    /**
     * An autocomplete method for the resources file formats. Uses the OD-Registrie's autoSuggest method.
     * 
     * @param fragment
    ...
    /**
     * An autocomplete method for the app used datasets. Uses the OD-Registrie's autoSuggest method.
     * 
     * @param fragment
     *            the fragment
     * @return the list
    ...
    /**
     * Adds a new empty resource to the resource list. This can be used to create a new resource for the metadataController.getMetadata().
     * 
     * @return null
     */
